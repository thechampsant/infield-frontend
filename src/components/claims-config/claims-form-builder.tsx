...
type BuilderFieldTypeOption = {
  key: UdfFieldType;
  label: string;
  description: string;
};

const FALLBACK_FIELD_TYPES: BuilderFieldTypeOption[] = [
...
export function ClaimsFormBuilder({
  projectId,
  claimTypeName,
  fields,
  savedFields,
  ...
  const [fieldTypes, setFieldTypes] = useState<BuilderFieldTypeOption[]>(FALLBACK_FIELD_TYPES);
  ...
  useEffect(() => {
    ...
    async function loadMeta() {
      ...
      setSourceError(null);
      try {
        const [sourceList, backendFieldTypes] = await Promise.all([
          udfConfigService.getSources(),
          udfConfigService.getFieldTypes(),
        ]);
        if (cancelled) return;
        setSources(sourceList);
        if (backendFieldTypes.length > 0) {
          setFieldTypes(
            backendFieldTypes.map((item) => ({
              key: item.key,
              label: item.label,
              description: item.label.replaceAll("_", " "),
            })),
          );
        }
      } catch (err) {
        if (!cancelled) {
          setSourceError(formatApiError(err, "Failed to load UDF metadata"));
        }
      } finally {
    ...
    fields.forEach((field, index) => {
      ...
      const config =
        field.config && typeof field.config === "object"
          ...
          : {};
      const sourceKey = String(config.dataSource ?? "").trim();
      if (!sourceKey || sourcePreviewByField[index]) return;
      void loadSourcePreview(index, sourceKey);
    });
    // Preload previews for saved API-backed fields when the selected claim type changes.
  ...
  function removeField(index: number) {
    ...
    updateFields(next);
    setSourcePreviewByField((prev) => {
      const nextPreview: Record<number, UdfSourcePreviewItem[]> = {};
      Object.entries(prev).forEach(([key, rows]) => {
        const currentIndex = Number(key);
        if (currentIndex < index) nextPreview[currentIndex] = rows;
        if (currentIndex > index) nextPreview[currentIndex - 1] = rows;
      });
      return nextPreview;
    });
  }
  ...
  async function loadSourcePreview(index: number, sourceKey: string) {
    ...
    try {
      ...
      setSourceError(formatApiError(err, "Failed to load source preview"));
      setSourcePreviewByField((prev) => ({ ...prev, [index]: [] }));
    } finally {
  ...
  return (
    <div className="att-form-builder claims-form-builder">
      <div className="att-form-builder__head">
        <div>
          <div className="att-form-builder__eyebrow">Step 3 · Claim Form Builder</div>
          <h2 className="att-form-builder__title">{claimTypeName} Form</h2>
          <p className="att-form-builder__desc">
            Define the additional UDF fields users must complete when they submit a{" "}
            {claimTypeName.toLowerCase()} claim. Static fields `submittedValue` and `claimDate`
        ...
        <div className="att-form-builder__meta">
          ...
          <button className="btn btn-primary" onClick={onSave} disabled={saving || loading}>
            {saving ? "Saving…" : `Save ${claimTypeName} Form`}
          </button>
        </div>
      </div>
      ...
      <div className="claims-form-builder__staticFields">
        <div className="claims-form-builder__staticField">
          <strong>submittedValue</strong>
          <span>NUMBER · claim amount</span>
        </div>
        <div className="claims-form-builder__staticField">
          <strong>claimDate</strong>
          <span>DATE · date of expense</span>
        </div>
      </div>
      ...
      <div className="att-form-builder__layout">
        ...
        <div className="att-form-builder__canvas">
          ...
          {loading ? (
            <div className="att-form-builder__empty">Loading claim form…</div>
          ) : fields.length === 0 ? (
            <div className="att-form-builder__empty">
              No additional fields yet. Add a field from the palette or keep only the default
              amount/date fields.
            </div>
          ) : (
                ...
                <>
                  <label className="att-form-builder__field">
                    ...
                    <select
                      className="form-input"
                      value={String(
                        ((selectedField.config as Record<string, unknown> | undefined)?.dataSource ??
                          ""),
                      )}
                      onChange={(e) => {
                  ...
                  <div className="att-form-builder__fieldRow">
                    <label className="att-form-builder__field">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateFieldConfig(selectedIndex, { labelField: e.target.value })
                        }
                      />
                    </label>
                    <label className="att-form-builder__field">
